...
from Alex_model3 import AlexNet  # 确保 AlexNet 输出层调整为回归任务（num_classes=1）
...
class NormalizedLoss(nn.Module):
    def __init__(self, temp_mean, temp_std):
...
def main():
    ...
    tem_labels = torch.tensor(train_dataset.labels)
    tem_mean, tem_std = tem_labels[:, 0].mean().item(), tem_labels[:, 0].std().item()
    print(f"Time mean: {tem_mean}, std: {tem_std}")

    net = AlexNet(num_outputs=1, init_weights=True)
    net.to(device)

    loss_function = NormalizedLoss(tem_mean, tem_std)
    optimizer = optim.Adam(net.parameters(), lr=0.0002)

    epochs = 100
    save_path = 'AlexNet_regression_3000_3.pth'
    best_loss = float('inf')
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            tem_preds.append(outputs)
            tem_labels.append(labels)

        tem_preds = torch.cat(tem_preds, dim=0)
        tem_labels = torch.cat(tem_labels, dim=0)
        train_r2 = calculate_r2(tem_preds, tem_labels)
