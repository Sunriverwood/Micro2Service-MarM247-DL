...
class NormalizedLoss(nn.Module):
    def __init__(self, temp_mean, temp_std, time_mean, time_std,delta=1):
    ...
    def forward(self, predictions, targets):
        ...
        temp_loss = nn.functional.huber_loss(temp_pred, temp_target, delta=self.delta)
        time_loss = nn.functional.huber_loss(time_pred, time_target, delta=self.delta)
...
def main():
    ...
    net = InceptionV3(num_outputs=2, pretrained=True, dropout_p=0.5, task_type='regression')
    ...
    epochs = 1000
    save_path = './parameter/filter-reg-inception.pth'
    best_loss = float('inf')
